...
int main(int argc, char* argv[]) {
    ...
    TTF_Font* font = TTF_OpenFont("assets/Jersey.ttf", 24);
    if (!font) {
        printf("Chyba fontu: %s\n", TTF_GetError());
    }
    
    TTF_Font* largeFont = TTF_OpenFont("assets/Jersey.ttf", 48);
    if (!largeFont) {
        printf("Chyba fontu: %s\n", TTF_GetError());
    }

    TTF_Font* massiveFont = TTF_OpenFont("assets/Jersey.ttf", 96);
    if (!massiveFont) {
        printf("Chyba fontu: %s\n", TTF_GetError());
    }

    Settings settings;
    loadSettings(&settings, "assets/settings.txt");
    ...
    while (quit == false)
    {
        while (SDL_PollEvent(&event))
        {
            ...
            if (state == SETTINGS) {
                ...
                if (buttonClicked(&inputBtn, &event)) {
                    ...
                    strcpy(inputBtn.text, settings.input == ARROWS ? "ARROWS" : "WASD");
                    saveSettings(&settings, "assets/settings.txt");
                }
        ...
        if (state == MENU) {
            renderText(renderer, massiveFont, "TETRIS", text, 170, 100);
